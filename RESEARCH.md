...
> **Tarih:** 2026-07-28
> **Kapsam:** Sadece Arkoz Müteahhitlik İnşaat Taahhüt Turizm ve Dış Tic. A.Ş.
...
## 14. Teknik Referans — Bu Proje İçin Seçilen Yığın

Senin tercihin: **saf HTML/CSS/JS + GitHub Pages** (build adımı yok).

Mevcut Arkoz repo'nda işe yarayan ve bu projede de uygulanabilir olan teknik pratikler:
- **BEM** CSS metodolojisi (`blok__element--modifier`)
- CSS değişkenleri `:root` içinde
- Animasyonlarda `transform` ve `opacity` (GPU hızlandırma)
- Breakpoint'ler: `768px` (tablet), `480px` (mobil)
- Tüm `<img>` etiketlerinde `alt` + `loading="lazy"`
- `<script>` etiketleri `defer` ile veya `</body>` öncesinde
- Erişilebilirlik: `aria-label`, `aria-hidden`
- Deploy: GitHub Actions → GitHub Pages

⛔ **Geçmiş oturumlardan sert kural:** animasyonlar, renkler ve görsel efektler bir kez onaylandıktan sonra değiştirilmez. Bu kural iki ayrı oturumda revert ile pekiştirilmiş. Ayrıca istenmeden ek özellik / "iyileştirme" eklenmez, API anahtarı JS dosyasına yazılmaz.

**Marka kimliği:** Arkoz Müteahhitlik ayrı bir şirket olduğu için **kendi görsel kimliği** olacak. Renk paleti, tipografi ve logo senden gelecek ya da birlikte belirlenecek.

---

## 15. Açık Sorular — Kullanıcıdan Beklenen

### 🔴 Site yapılmadan önce şart olan tek soru
**Hedef kitle kim?** Rakip analizinde iki arketip neredeyse hiç örtüşmüyor (bkz. böl. 1):
- **Arsa sahibi** (kat karşılığı / kentsel dönüşüm) → Hizmetler, Süreç, Bölgeler, Teklif Al
- **Daire alıcısı** → Projeler, Kurumsal, Basın, Kariyer
- **Kamu ihalesi / kurumsal müşteri** → Referanslar, Kapasite, Belgeler

Bu cevap tüm site mimarisini belirliyor.

### Site yazılırken lazım olacaklar
- [ ] **Yetki belge numarası ve grubu** — varsa en değerli içerik *(üstünlük iddiası olarak değil, şeffaflık olarak — md. 7/6)*
- [ ] Faaliyet adresi + Google Maps linki *(DAİB kaydı Ağrı diyor, sen Ümraniye dedin — netleşmeli)*
- [ ] Kuruluş tarihi / faaliyet süresi
- [ ] Geçmiş projeler — ada/parsel, m², birim sayısı, ruhsat/iskân tarihleri
- [ ] Fotoğraflar
- [ ] Logo / marka dosyaları, renk tercihi
- [ ] Künye için: vergi no, Mersis no, ticaret sicil no, KEP adresi
- [ ] İletişim: telefon, e-posta
- [ ] KVKK: veri sorumlusu kimliği, VERBİS no (varsa), başvuru e-postası
